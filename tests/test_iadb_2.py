...
class SmolBlock(torch.nn.Module):
    ...
    def __init__(self, out_channels, in_channels, stride, kernel_size=3, padding=1) -> None:
        super().__init__()
        ...
        # self.activation     = SmolActivation()
    ...
    def forward(self, i):
        ...
        # apply spline
        x = self.activation(x)
        # x = self.spline_k[0] + x * self.spline_k[1] + x**2 * self.spline_k[2] + x**3 * self.spline_k[3]
        return x
...
class SmolUnet(torch.nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.smol_block_down_1          = SmolBlock(out_channels=32, in_channels=self.num_features_1, kernel_size=3, stride=1)
        self.smol_block_down_2          = SmolBlock(out_channels=64, in_channels=32, kernel_size=3, stride=1)
        self.smol_block_down_3          = SmolBlock(out_channels=128, in_channels=64, kernel_size=3, stride=1)
        ...
        self.feature_reconstruction_conv = torch.nn.Conv2d(in_channels=self.num_features_1, out_channels=self.num_output_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, alpha):
        ...
        x4 = torch.nn.Upsample(scale_factor=2, mode='bilinear')(_x4)
        ...
        x10 = self.feature_reconstruction_conv(x9)
        return x10
